...
try:
  for article in section_guardian.find_all("a", class_="u-faux-block-link__overlay js-headline-text"):
    try:
      ...
      News.objects.update_or_create(source="Guardian", headline=title_guardian, link=href_guardian, defaults={"content":final_content_guardian, "summary":summary_guardian_text})
  ...
  for articles in list_bbc.find_all("article"):
    try:
      ...
      if len(posted_bbc) == 1: #identifies todays posts
        ...
        #insert in database
        News.objects.update_or_create(source="bbc", headline=title_bbc, link=href_bbc, defaults={"content":final_content_bbc, "summary":summary_bbc_text}) 
    except Exception as e:
      pass
except Exception as e:
  pass 
...
try:
  ...
  for links in section_reuters.find_all("li"):
      ...
      if "AM" in posted_reuters or "PM" in posted_reuters:
        ...
        #insert in database
        News.objects.update_or_create(source="Reuters", headline=title_reuters, link=href_reuters, content=final_content_reuters, defaults={"content":final_content_reuters, "summary":summary_reuters_text})
except Exception as e:
  pass
